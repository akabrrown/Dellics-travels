...
export async function loginAdminAccountInit(
  email: string,
  password?: string
): Promise<{ success: boolean; requireOtp?: boolean; error?: string }> {
  try {
    const { adminApi } = require("./api");
    const loginRes = await adminApi.post("/auth/admin/login-init", {
    ...
    console.error("Backend auth init error:", err);
    return { 
      success: false, 
